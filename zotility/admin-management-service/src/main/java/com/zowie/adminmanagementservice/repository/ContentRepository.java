...
public class ContentRepository  {
    ...
    public Content delete(String id) {
        Content content = null;
        try {
            content = contentTable.deleteItem(Key.builder().partitionValue(id).build());
        }
        catch (DynamoDbException e) {
            logger.error(e.getMessage());
        }
    ...
    public Content update(Content content) {
        try {
            return contentTable.updateItem((UpdateItemEnhancedRequest.Builder<Content> requestBuilder) -> requestBuilder.item(content));
        }
        catch (DynamoDbException e) {
            logger.error(e.getMessage());
            return null;
